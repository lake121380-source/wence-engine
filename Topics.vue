  ...
  <div>
    ...
    <div class="page-header">
      <div>
        ...
        <div class="page-subtitle">关键词搜索爆款短视频，采用后一键发起文案生�?/div>
      </div>
    ...
    <div class="search-card">
      ...
      <div v-if="videoType" style="margin-top:8px;color:#94a3b8;font-size:13px;">
        当前为热榜模式，数据来自抖音{{ currentVideoTypeLabel }}，排序和发布时间由热榜接口决
      </div>
    </div>
        ...
        <div v-if="historyKeywords.length" class="recent-searches">
          ...
          <div class="recent-list">
            ...
            >
              ...
              <span class="recent-meta">{{ h.count }} 条结</span>
            </div>
          </div>
        </div>
      </div>
          ...
          >
            ...
            <div class="video-body">
              ...
              <div class="card-actions">
                ...
                <n-button
                  v-if="v._hasAnalysis || v._analysis"
                  size="tiny"
                  secondary
              ...
              <div class="st-card" :class="{ 'st-card-selected': batchMode && savedSelectedIds.includes(t.id) }" @click="batchMode && toggleSavedSelect(t.id)">
                ...
                <div class="st-body">
                  ...
                  <div class="st-actions">
                    ...
                    <n-button v-if="t.has_analysis || t._analysis" size="tiny" secondary @click="viewSavedAnalysis(t)">查看分析</n-button>
                    <n-dropdown
                      :options="[
                        { label: '已采纳', key: '已采纳' },
                        { label: '已忽略', key: '已忽略' },
...
async function analyzeVideo(v) {
  ...
  if (!v.script || v.script === v.title) {
    message.info('正在获取视频语音内容，首次分析约 30-60 秒，请稍..', { duration: 8000 })
  }
  try {
    ...
    if (idx !== -1) results.value[idx] = { ...results.value[idx], _analysis: mergedAnalysis, _hasAnalysis: true }
    currentAnalysis.value = mergedAnalysis
...
async function batchAnalyzeSelected() {
  ...
  message.info(`始批量分${ids.length} 条内..`)
  try {
...
async function batchFetchSelectedDetails() {
  ...
  if (ok) message.success(`已获${ok} 条因无文案`)
  if (fail) message.warning(`${fail} 条获取失`)
}
...
async function confirmAddToDocs() {
  ...
  if (ok) message.success(`已添${ok} 条文案到资料库`)

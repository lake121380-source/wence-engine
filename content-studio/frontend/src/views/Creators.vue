<template>
  <div class="creators-page">
    ...
    <div class="page-header">
      <div class="header-left">
        <div class="title-stack">
          <div class="page-kicker">Creator Intelligence Hub</div>
          <h1 class="page-title">博主资料</h1>
          <div class="page-subtitle">从发现到抓取，再到风格提炼与爆款诊断，一页完成创作前准备</div>
        </div>
        <div class="page-meta" v-if="creators.length > 0">
          <span class="meta-badge">{{ creators.length }} 位博主</span>
          <span class="meta-dot"></span>
          <span class="meta-item">{{ styledCount }} 已提取风格</span>
          <span class="meta-dot"></span>
          <span class="meta-item">{{ totalVideoCount }} 条视频</span>
        </div>
      </div>
      <div class="header-actions">
        <n-button secondary size="small" :disabled="compareIds.length < 2" @click="openCompareModal">
          <template #icon><n-icon><ColorPaletteOutline /></n-icon></template>
          风格对比 ({{ compareIds.length }})
        </n-button>
        <n-button secondary size="small" @click="showAutoModal = true">
    ...
    <div class="filter-bar">
      <n-tabs v-model:value="activePlatform" type="segment" animated size="small">
      ...
      <div class="filter-tools">
        <n-input v-model:value="searchKeyword" clearable size="small" placeholder="搜索昵称 / 账号ID / 平台">
          <template #prefix>
    ...
    <n-spin :show="loading">
      <div v-if="filteredCreators.length === 0" class="empty-state">
        <n-icon size="40" color="#d1d5db"><PeopleOutline /></n-icon>
        <p>{{ searchKeyword ? '没有匹配到博主' : '还没有添加博主' }}</p>
        <n-button type="primary" size="small" @click="showAddModal = true">添加第一个博主</n-button>
      </div>

      <div v-else class="cards-grid">
        <div v-for="c in filteredCreators" :key="c.id" class="creator-card">
          <!-- 平台色条 -->
          <div class="card-stripe" :class="'stripe-' + c.platform"></div>
          <!-- 主信息行 -->
          <div class="card-main">
            <div class="avatar-wrap">
              ...
              />
              <span v-if="c.has_style" class="avatar-badge" title="已提取风格"></span>
            </div>
            <div class="card-info">
              <div class="card-name-row">
                <span class="creator-name">{{ c.nickname }}</span>
                <n-tag size="tiny" :type="platformType(c.platform)" :bordered="false" round>{{ c.platform }}</n-tag>
              </div>
          ...
          <div class="card-stats">
            <div class="stat-item">
              <span class="stat-val">{{ formatNum(c.follower_count) }}</span>
              <span class="stat-label">粉丝</span>
            </div>
            <div class="stat-sep"></div>
            <div class="stat-item">
              <span class="stat-val">{{ c.videos_in_db ?? 0 }}</span>
              <span class="stat-label">已抓</span>
            </div>
            <div class="stat-sep"></div>
            <div class="stat-item">
              <span class="stat-val" :class="c.last_crawled_at ? 'val-green' : 'val-gray'">{{ c.last_crawled_at ? '已更新' : '未抓取' }}</span>
          ...
          <div class="card-actions">
            <n-button type="primary" size="small" style="flex:1" @click.stop="viewVideos(c)">
              <template #icon><n-icon><FilmOutline /></n-icon></template>
              查看视频
            </n-button>
            <n-popover trigger="hover" placement="bottom">
              <template #trigger>
                <n-button size="small" secondary style="flex:1" :loading="crawlingId === c.id" @click="crawl(c)">
                  <template #icon><n-icon><RefreshOutline /></n-icon></template>
                  抓取内容
                </n-button>
              </template>
              <div style="display:flex;align-items:center;gap:4px;padding:4px 2px;">
                <span style="font-size:11px;color:#aaa;margin-right:2px;">数量</span>
                <n-button
          ...
          <div class="card-footer">
            <n-button
              size="tiny"
              text
              :disabled="!canAnalyzeStyle(c)"
              :loading="analyzingId === c.id"
              @click="analyzeStyle(c)"
            >
              <template #icon><n-icon><ColorPaletteOutline /></n-icon></template>
              {{ c.has_style ? '重提风格' : '提取风格' }}
            </n-button>
            <span v-if="!canAnalyzeStyle(c)" class="style-gate-tip">需先分析 {{ styleProgressText(c) }}</span>
            <n-button size="tiny" text @click="toggleCompare(c)">
              <template #icon><n-icon><ColorPaletteOutline /></n-icon></template>
              {{ isCompared(c.id) ? '取消对比' : '加入对比' }}
            </n-button>
            <n-button size="tiny" text @click="viewIntelCard(c)">
              <template #icon><n-icon><DocumentOutline /></n-icon></template>
              情报卡
            </n-button>
            <span v-if="c.last_crawled_at" class="footer-date">{{ formatDate(c.last_crawled_at) }}</span>
            <n-button size="tiny" text style="color:#ef4444;margin-left:auto" @click="confirmDelete(c)">
    ...
    <n-modal v-model:show="showAddModal" preset="card" title="添加博主" style="width: 520px;">
      <n-form ref="formRef" :model="form" :rules="rules" label-placement="top">
        ...
        <template v-if="form.platform === 'weixin'">
          ...
          <div v-else-if="weixinSearched && !weixinSearching" style="padding:12px 0;text-align:center;color:rgba(0,0,0,0.3);font-size:13px;">
            未找到相关博主，请尝试其他关键词
          </div>
        </template>
        <!-- 其他平台：手动输入-->
    ...
    <n-modal v-model:show="showAutoModal" preset="card" title="一键发现头部博主" style="width: 520px;">
      <n-form label-placement="top" :show-feedback="false">
        ...
        <n-form-item label="发现数量">
          <n-radio-group v-model:value="autoForm.limit" button-style="solid">
            <n-radio-button :value="10">10条</n-radio-button>
            <n-radio-button :value="20">20条</n-radio-button>
            <n-radio-button :value="30">30条</n-radio-button>
          </n-radio-group>
        </n-form-item>
      </n-form>
      ...
      <div v-if="autoTaskId" class="auto-progress">
        ...
        <div class="progress-status">
          ...
          <span style="margin-left: 8px; font-size: 12px; color: rgba(0,0,0,0.4);">
            {{ autoTask.progress }}/{{ autoTask.total }} 位博主          </span>
        </div>
    ...
    <n-modal v-model:show="showCompareModal" preset="card" style="width: 980px; max-width: 95vw; max-height: 82vh; overflow-y: auto;">
      <template #header>风格对比（最多 3 位）</template>

      <n-spin :show="compareLoading">
        <div v-if="compareCards.length" class="compare-grid">
          <div v-for="item in compareCards" :key="item.creator.id" class="compare-card">
            <div class="compare-card-head">
              <n-avatar :src="item.creator.avatar_url" :size="34" round />
              <div class="compare-name-wrap">
                <div class="compare-name">{{ item.creator.nickname }}</div>
                <div class="compare-meta">{{ platformLabel(item.creator.platform) }} · {{ formatNum(item.creator.follower_count) }} 粉丝</div>
              </div>
            </div>

            <template v-if="item.intel">
              <div class="compare-section">
                <div class="compare-label">账号定位</div>
                <div class="compare-text">{{ clipText(item.intel.positioning, 120) }}</div>
              </div>
              <div class="compare-section">
                <div class="compare-label">视频风格</div>
                <div class="compare-text">{{ clipText(item.intel.video_style, 120) }}</div>
              </div>
              <div class="compare-section">
                <div class="compare-label">爆款公式</div>
                <div class="compare-text">{{ clipText(item.intel.viral_formula || item.intel.summary, 120) }}</div>
              </div>
            </template>
            <template v-else>
              <div class="compare-empty">暂无情报卡，请先在该博主卡片中点击「情报卡」生成。</div>
            </template>
          </div>
        </div>
        <div v-else class="empty-intel">
          <n-empty description="请先选择至少两位博主" />
        </div>
      </n-spin>

      <template #footer>
        <n-space justify="end">
          <n-button @click="clearCompare">清空选择</n-button>
          <n-button type="primary" @click="showCompareModal = false">关闭</n-button>
        </n-space>
      </template>
    </n-modal>
              ...
              >
                ...
                <div class="cv-cover">
                  ...
                  <div v-else class="cv-cover-ph">
                    <n-icon size="24" color="rgba(0,0,0,.2)">
                  ...
                  <div v-else-if="v.note_type === 'video'" class="cv-type-badge" style="background:rgba(37,99,235,.75)">视</div>
                </div>
    ...
    <n-modal v-model:show="showCVAnalysisModal" preset="card" style="width: 720px;">
      ...
      <div v-if="currentCVAnalysis" class="analysis-modal">
        ...
        <div v-if="currentCVVideo" class="cv-anal-meta">
          ...
          <span class="cam-item"><n-icon><ChatbubbleOutline /></n-icon> {{ formatNum(currentCVVideo.comment_count) }} 评</span>
          ...
          <a v-if="currentCVVideo.video_url" :href="currentCVVideo.video_url" target="_blank" rel="noopener noreferrer" class="cam-link">
            {{ currentCVVideo.note_type !== undefined ? '原笔记' : '原视频' }}
          </a>
        </div>
        ...
        <n-tabs type="line" animated>
          ...
          <n-tab-pane name="script" tab="视频文稿">
...
const STYLE_ANALYSIS_MIN_VIDEOS = 30

function analyzedVideosCount(creator) {
  return Number(creator?.analyzed_videos_count || 0)
}

function canAnalyzeStyle(creator) {
  return analyzedVideosCount(creator) >= STYLE_ANALYSIS_MIN_VIDEOS
}

function styleProgressText(creator) {
  return `${analyzedVideosCount(creator)}/${STYLE_ANALYSIS_MIN_VIDEOS}`
}

const styledCount = computed(() => creators.value.filter(c => c.has_style).length)
const totalVideoCount = computed(() => creators.value.reduce((s, c) => s + (c.videos_in_db || 0), 0))

const filteredCreators = computed(() => {
  const byPlatform = activePlatform.value === 'all'
    ? creators.value
...
async function load() {
  ...
  try {
    ...
    creators.value = data
    compareIds.value = compareIds.value.filter((id) => creators.value.some((x) => x.id === id))
  } finally {
    loading.value = false
  }
}
...
function toggleCompare(c) {
  ...
  if (compareIds.value.length >= 3) {
    message.warning('最多选择 3 位博主进行对比')
    return
  }
...
async function openCompareModal() {
  if (compareIds.value.length < 2) {
    message.warning('至少选择 2 位博主才能对比')
    return
  }
  ...
  try {
    const selected = compareIds.value
      .map((id) => creators.value.find((x) => x.id === id))
      .filter(Boolean)

    const rows = await Promise.all(
      selected.map(async (creator) => {
        try {
          const { data } = await analyzerApi.getIntelCard(creator.id)
          return { creator, intel: data }
        } catch {
          return { creator, intel: null }
        }
      })
    )

    compareCards.value = rows
  } finally {
...
async function analyzeStyle(c) {
  if (!canAnalyzeStyle(c)) {
    message.warning(`需先完成至少 ${STYLE_ANALYSIS_MIN_VIDEOS} 条视频分析（当前 ${analyzedVideosCount(c)} 条）`)
    return
  }
...
async function startAutoDiscover() {
  ...
  try {
    ...
    message.error(e.response?.data?.detail || '自动发现失败')
...
function statusLabel(s) {
  return { pending: '排队中', running: '运行中', done: '已完成', error: '出错', idle: '待开始' }[s] || s
}
...
function displayUid(c) {
  ...
  if (!uid) return ''
  // 视频号 unique_id 形如 v2_060000...@finder，直接截断展示
...
function fmtRatio(v) {
  ...
  if (v > 1) return '无播放数据'  // play_count=0 时属脏数据
  return (v * 100).toFixed(1) + '%'
}
function formatDate(d) {
  if (!d) return ''
  // 如果字符串不含时区标识则加 Z 让浏览器正确识别为本地时间
  const raw = String(d)
...
const currentCVVideo = ref(null)   // 当前打开分析弹窗的视频
...
.page-header {
  ...
  box-shadow: 0 2px 8px rgba(8,145,178,0.06), 0 0 0 1px rgba(8,145,178,0.03);
  margin-bottom: 14px;
}
...
.stripe-douyin      { background: linear-gradient(90deg, #fe2c55, #ff8080); }
.stripe-xiaohongshu { background: linear-gradient(90deg, #ff2442, #ff8080); }
.stripe-weixin      { background: linear-gradient(90deg, #07c160, #4ade80); }
...
.weixin-result-item:hover { background: #f8fafc; }
.weixin-result-item.active { background: #eef2ff; }
.weixin-result-info { flex: 1; min-width: 0; }
.weixin-result-name { font-size: 14px; font-weight: 500; color: #111827; }
.weixin-result-desc { font-size: 12px; color: #6b7280; margin-top: 2px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.weixin-avatar { width: 36px; height: 36px; border-radius: 50%; overflow: hidden; background: #ecfeff; flex-shrink: 0; display: flex; align-items: center; justify-content: center; }
.weixin-avatar-img { width: 100%; height: 100%; object-fit: cover; display: block; }
.weixin-avatar-fallback { font-size: 14px; font-weight: 700; color: var(--brand); }

/* ═══ 情报卡 ═══ */
.intel-section { font-size: 14px; color: #334155; line-height: 1.8; padding: 8px 0; white-space: pre-wrap; }
.intel-tags { padding: 8px 0; }
.pain-item { background: #f8fafc; border-radius: var(--radius-sm); padding: 12px 14px; margin-bottom: 10px; border: 1px solid var(--line-weak); }
.pain-title { font-size: 14px; font-weight: 600; color: #0f172a; margin-bottom: 4px; }
...
/* ═══ 自动发现 ═══ */
.auto-progress { margin-top: 16px; padding: 16px; background: #f8fafc; border-radius: var(--radius-md); border: 1px solid var(--line-weak); }
...
/* ═══ 博主视频抽屉 ═══ */
.cv-card { background: #fff; border: 1px solid var(--line); border-radius: var(--radius-sm); overflow: hidden; cursor: pointer; transition: border-color .2s, box-shadow .15s; position: relative; }
.cv-card:hover { border-color: #a5b4fc; box-shadow: 0 3px 12px rgba(99,102,241,.08); }
.cv-card.selected { border-color: #6366f1; background: #fafafe; }
...
.cv-type-badge { position: absolute; top: 6px; left: 6px; font-size: 10px; line-height: 1; padding: 2px 6px; border-radius: 6px; background: rgba(2, 6, 23, 0.6); color: #fff; }
.cv-cover-ph { display: flex; align-items: center; justify-content: center; width: 100%; height: 100%; }
.cv-body { padding: 10px; }
.cv-title { font-size: 13px; font-weight: 500; color: #0f172a; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; overflow: hidden; line-height: 1.4; margin-bottom: 6px; }
.cv-stats { display: flex; gap: 10px; margin-bottom: 6px; }
.cstat-s { display: flex; align-items: center; gap: 3px; font-size: 12px; color: #94a3b8; font-variant-numeric: tabular-nums; }
.ratio-badge { background: #e0f2fe; color: var(--brand-strong); border-radius: 6px; padding: 0 5px; font-size: 11px; }
...
.batch-task-text { font-size: 12px; color: rgba(0,0,0,0.55); white-space: nowrap; }
...
.cv-anal-meta { display: flex; align-items: center; gap: 16px; flex-wrap: wrap; background: #f8fafc; border-radius: var(--radius-sm); padding: 10px 14px; margin-bottom: 14px; border: 1px solid var(--line-weak); }
.cam-item { display: flex; align-items: center; gap: 4px; font-size: 13px; color: #475569; }
.cam-link { font-size: 13px; color: #6366f1; text-decoration: none; margin-left: auto; }
...
.ratio-value { font-size: 20px; font-weight: 700; color: #0f172a; font-variant-numeric: tabular-nums; }
.analysis-text { font-size: 14px; color: #334155; line-height: 1.8; padding: 8px 0; white-space: pre-wrap; }
.analysis-highlight { background: #f0fdfa; border-left: 3px solid #14b8a6; padding: 12px 16px; border-radius: 8px; }

/* ═══ 输入框优化 ═══ */
:deep(.n-input) { border-radius: var(--radius-sm) !important; }
:deep(.n-input--focus) { box-shadow: 0 0 0 3px rgba(2,132,199,0.12) !important; }
:deep(.n-base-selection) { border-radius: var(--radius-sm) !important; }
:deep(.n-modal-card) { border-radius: var(--radius-md) !important; }
:deep(.n-tabs .n-tabs-tab) { border-radius: 8px; }
:deep(.n-tabs .n-tabs-tab--active) { color: var(--brand-strong) !important; }
:deep(.n-button) { border-radius: var(--radius-sm); }
:deep(.n-form-item-label) { font-weight: 500; font-size: 13px; color: #374151; }

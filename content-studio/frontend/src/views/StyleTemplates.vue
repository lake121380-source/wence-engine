  ...
  <div>
    <div class="page-header">
      <div>
        ...
        <div class="page-subtitle">管理博主风格，用于文案生成时的风格模</div>
      </div>
      <n-space>
    ...
    <n-alert type="info" :bordered="false" style="margin-bottom: 20px; border-radius: 10px;">
      提示：在「博主资料库」页面抓取博主内容后，点击「提取风格」可自动生成风格模版    </n-alert>
    ...
    <n-spin :show="loading">
      <div v-if="templates.length === 0 && !loading" class="empty-state-big">
        <n-icon size="48" color="rgba(0,0,0,0.15)"><ColorPaletteOutline /></n-icon>
        <p>还没有风格模版</p>
        ...
        <n-grid :cols="2" :x-gap="16">
          <n-gi>
            <n-form-item label="模版名称">
              <n-input v-model:value="form.name" placeholder="例如：美妆干货博主" />
            </n-form-item>
          </n-gi>
          <n-gi>
    ...
    <n-modal v-model:show="showCombinedModal" preset="card" title="多博主联合风格分析" style="width: 560px;">
      <n-alert type="info" :bordered="false" style="margin-bottom: 16px; border-radius: 8px;">
        选择多个博主，AI 将分析他们的共同风格特征，生成融合风格模板。建议先在「博主资料库」完成内容抓取      </n-alert>
...
async function analyzeCombined() {
  if (!combinedForm.value.template_name) { message.warning('请填写模板名'); return }
  if (combinedForm.value.creator_ids.length < 1) { message.warning('请至少选择一个博'); return }

    ...
    <n-modal v-model:show="showDetailModal" preset="card" style="width: 720px; max-height: 88vh;">
      ...
      <n-spin :show="detailLoading">
        <div v-if="detailDoc">
          ...
          <div v-if="detailDoc.ai_summary" class="detail-ai-summary">
            <div class="detail-ai-title">AI理解摘要</div>
            <div class="detail-ai-text">{{ detailDoc.ai_summary }}</div>
          </div>
...
async function openDetail(row) {
  ...
  try {
    ...
    detailDoc.value = { ...row, ...(docRes?.data || {}) }

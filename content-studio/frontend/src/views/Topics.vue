<template>
  <div>
    ...
    <div class="page-header">
      <div>
        <div class="page-title">爆款</div>
        <div class="page-subtitle">关键词搜索爆款短视频，选中后可一键发起文案生成</div>
      </div>
      ...
      <n-button type="primary" ghost style="margin-left:8px;" @click="showUrlModal = true">
    ...
    <!-- 链接添加 Modal -->
    <n-modal v-model:show="showUrlModal" preset="card" title="通过链接添加爆款选题" style="width: 560px;">
      <div style="margin-bottom:12px;color:#64748b;font-size:13px;">
        粘贴抖音 / 小红书 的视频链接或分享文案，系统会自动识别并保存到选题库。
      </div>
      <n-input
        v-model:value="urlInput"
        type="textarea"
        placeholder="例如：https://v.douyin.com/xxxxx  或直接粘贴分享的整段文字"
        :autosize="{ minRows: 3, maxRows: 5 }"
        :disabled="addingByUrl"
      />
      <div style="display:flex;justify-content:flex-end;gap:8px;margin-top:16px;">
        <n-button @click="showUrlModal = false" :disabled="addingByUrl">取消</n-button>
        <n-button type="primary" :loading="addingByUrl" @click="submitUrlAdd">
          解析并保存
        </n-button>
      </div>
    </n-modal>
    ...
    <div class="search-card">
      <n-form inline :label-width="0" style="gap:12px;flex-wrap:wrap;">
        <n-form-item label="搜索模式" label-style="color:#64748b;font-size:12px">
        ...
        <n-form-item style="flex:1;min-width:200px;">
          <n-input
            ...
            :placeholder="videoType ? '可输入关键词过滤热榜结果（选填）' : '输入行业关键词，如：美妆、减脂、护肤'"
            size="large"
            clearable
        ...
        <n-form-item label="平台" label-style="color:#64748b;font-size:12px">
          <n-checkbox-group v-model:value="platforms" style="display:flex;gap:8px;">
            <n-checkbox value="douyin">抖音</n-checkbox>
            <n-checkbox value="xiaohongshu" :disabled="!!videoType">小红书</n-checkbox>
            <n-checkbox value="weixin" :disabled="!!videoType">视频号</n-checkbox>
          </n-checkbox-group>
        </n-form-item>

        <n-form-item v-if="!videoType" label="排序" label-style="color:#64748b;font-size:12px">
          <n-select v-model:value="sort" :options="sortOptions" style="width:120px;" />
        </n-form-item>

        <n-form-item v-if="!videoType" label="发布时间" label-style="color:#64748b;font-size:12px">
          <n-select v-model:value="publishTime" :options="publishTimeOptions" style="width:120px;" />
        </n-form-item>

        <n-form-item label="数量" label-style="color:#64748b;font-size:12px">
      ...
      <div v-if="videoType" style="margin-top:8px;color:#94a3b8;font-size:13px;">
        当前为热榜模式，数据来自抖音{{ currentVideoTypeLabel }}，排序和发布时间由热榜接口决定
      </div>
    </div>
          ...
          >
            ...
            <div class="video-cover">
              ...
              <div v-else class="cover-placeholder">
                <n-icon size="32" color="rgba(0,0,0,.2)"><VideocamOutline /></n-icon>
              </div>
              <div class="platform-tag">{{ platformZh(v.platform) }}</div>
              <div v-if="v._hasAnalysis" class="analyzed-badge">已分析</div>
            </div>
            ...
            <div class="video-body">
              ...
              <div v-if="v.tags?.length" class="video-tags">
                <n-tag
                  ...
                  size="tiny"
                  :bordered="false"
                  style="margin-right:4px;background:rgba(11,188,212,.15);color:#818cf8;"
                >
                  #{{ tag }}
                </n-tag>
              </div>

              <div v-if="v.viral_reason || v._analysis?.why_viral_summary" class="viral-reason">
                {{ v.viral_reason || v._analysis?.why_viral_summary }}
              </div>
              ...
              <div class="card-actions">
                ...
                <n-button
                  size="tiny"
                  secondary
                  @click.stop="openAddToDocsModal('search-single', [v])"
                >
                  <template #icon><n-icon><FolderOpenOutline /></n-icon></template>
                  加入资料库
                </n-button>
    ...
    <n-drawer v-model:show="showSavedDrawer" :width="840" placement="right">
      <n-drawer-content :title="`已保存（${savedTopics.length}）`" closable>
        <template #header-extra>
          ...
          <template v-if="batchMode">
            ...
            <n-button
              size="small"
              :disabled="savedSelectedIds.length === 0"
              @click="openAddToDocsModal('saved-batch', filteredSaved.filter(s => savedSelectedIds.includes(s.id)))"
              style="margin-right:8px;"
            >
              <template #icon><n-icon><FolderOpenOutline /></n-icon></template>
              加入资料({{ savedSelectedIds.length }})
            </n-button>
        ...
        <n-spin :show="loadingSaved">
          <div v-if="filteredSaved.length === 0" style="text-align:center;padding:60px 0;color:#888;">
            ...
            <n-gi v-for="t in filteredSaved" :key="t.id">
              <div class="st-card" :class="{ 'st-card-selected': batchMode && savedSelectedIds.includes(t.id) }" @click="batchMode && toggleSavedSelect(t.id)">
                ...
                <div class="st-cover">
                  ...
                  <div v-else class="st-cover-ph"><n-icon size="24" color="rgba(0,0,0,.2)"><VideocamOutline /></n-icon></div>
                  <div class="st-platform">{{ platformZh(t.platform) }}</div>
                  <div v-if="t.has_analysis" class="analyzed-badge">已分析</div>
                ...
                <div class="st-body">
                  ...
                  <div v-if="t.keyword" style="margin-bottom:6px;">
                    <n-tag size="tiny" :bordered="false" style="background:rgba(11,188,212,.1);color:#818cf8;">#{{ t.keyword }}</n-tag>
                  </div>

                  <div v-if="t.viral_reason || t._analysis?.why_viral_summary" class="st-viral-reason">
                    {{ t.viral_reason || t._analysis?.why_viral_summary }}
                  </div>
                  ...
                  <div class="st-actions">
                    ...
                    <n-button size="tiny" secondary :loading="savedAnalyzingIds.has(t.id)" @click="analyzeSavedVideo(t)">
                      <template #icon><n-icon><AnalyticsOutline /></n-icon></template>
                      {{ t._analysis ? '重新分析' : '分析爆款' }}
                    </n-button>
                    <n-button v-if="t.has_analysis" size="tiny" secondary @click="viewSavedAnalysis(t)">查看分析</n-button>
                    <n-button size="tiny" secondary @click.stop="openAddToDocsModal('saved-single', [t])">
                      <template #icon><n-icon><FolderOpenOutline /></n-icon></template>
                      加入资料库
                    </n-button>
                    <n-dropdown
    ...
    <n-modal v-model:show="showAddToDocsModal" preset="card" title="加入资料库" style="width: 420px;">
      <n-form label-placement="top" :show-feedback="false">
        ...
        <n-alert type="info" :bordered="false" style="border-radius:8px;font-size:13px;">
          将选中{{ selectedIds.length }} 条视频文案（已抓取文案的部分）添加到产品资料库，可在文案生成时引用
        </n-alert>
      </n-form>
      <template #footer>
        <n-space justify="end">
    ...
    <n-modal v-model:show="showAnalysisModal" preset="card" style="width: 680px;">
      ...
      <div v-if="currentAnalysis" class="analysis-modal">
        ...
        <div class="analysis-label">{{ analysisScriptLabel }}</div>
        <div class="analysis-script">{{ analysisScriptText || '暂无视频原文案，可先点击「获取原视频文案」再查看。' }}</div>
        ...
        <n-tabs type="line" animated>
          <n-tab-pane name="resonance" tab="点赞共鸣">
            <div class="analysis-label">博主说了什么让观众点赞？找到了哪些观点/金句/共鸣</div>
            <div class="analysis-text">{{ currentAnalysis.resonance_analysis || '暂无分析' }}</div>
          </n-tab-pane>
          <n-tab-pane name="discussion" tab="讨论钩">
          ...
          <n-tab-pane name="why" tab="爆款诊断">
            <div class="analysis-label">话题 · 观点 · 金句 · 共鸣 · 讨论钩 · 知识价值 · 值得复制的做法</div>
...
const limit = ref(_cachedCond?.limit || 10)
...
async function loadHistory(kw) {
  ...
  try {
    ...
    results.value = data.map(t => ({
      ...t,
      _saved: true,
      _hasAnalysis: !!t.has_analysis,
      _analysis: normalizeAnalysis(t.analysis, t),
    }))
  } catch {
    message.error('加载失败')
  } finally {
...
function rateRatio(value, key) {
  if (value == null) return 'low'
  const threshold = RATIO_THRESHOLDS[key]
...
function normalizeAnalysis(rawAnalysis, topic = {}) {
  if (!rawAnalysis) return null

  const analysis = { ...(rawAnalysis || {}) }
  if (analysis.like_play_ratio == null && topic.like_play_ratio != null) {
    analysis.like_play_ratio = topic.like_play_ratio
  }
  if (analysis.comment_play_ratio == null && topic.comment_play_ratio != null) {
    analysis.comment_play_ratio = topic.comment_play_ratio
  }
  if (analysis.collect_play_ratio == null && topic.collect_play_ratio != null) {
    analysis.collect_play_ratio = topic.collect_play_ratio
  }

  if (!analysis.like_play_level) {
    analysis.like_play_level = rateRatio(analysis.like_play_ratio, 'like_play')
  }
  if (!analysis.comment_play_level) {
    analysis.comment_play_level = rateRatio(analysis.comment_play_ratio, 'comment_play')
  }
  if (!analysis.collect_play_level) {
    analysis.collect_play_level = rateRatio(analysis.collect_play_ratio, 'collect_play')
  }

  if (!analysis.script && topic.script) {
    analysis.script = topic.script
  }
  if (!analysis.description && topic.description) {
    analysis.description = topic.description
  }
  if (!analysis.title && topic.title) {
    analysis.title = topic.title
  }
  return analysis
}

const analysisScriptText = computed(() => {
  const script = (currentAnalysis.value?.script || '').trim()
  if (script) return script
  return (currentAnalysis.value?.description || '').trim()
})

const analysisScriptLabel = computed(() => {
  const script = (currentAnalysis.value?.script || '').trim()
  if (script) return '视频原文案（语音转录）'
  const description = (currentAnalysis.value?.description || '').trim()
  if (description) return '视频文案（创作者描述）'
  return '视频原文案'
})

const sortOptions = [
  { label: '多点赞', value: 'likes' },
  { label: '最新发布', value: 'new' },
]
const videoTypeOptions = [
  { label: '不限（关键词搜索）', value: '' },
  { label: '低粉爆款榜', value: 'low_score_viral' },
  { label: '高完播率榜', value: 'high_completion' },
  { label: '高涨粉率榜', value: 'high_follower_growth' },
  { label: '高点赞率榜', value: 'high_likes' },
]
const currentVideoTypeLabel = computed(() => {
  const opt = videoTypeOptions.find(o => o.value === videoType.value)
...
async function search() {
  const usingBillboard = !!videoType.value
  if (!usingBillboard && !keyword.value.trim()) return message.warning('请输入关键词')
  if (!platforms.value.length) return message.warning('请选择至少一个平台')
  if (usingBillboard && !platforms.value.includes('douyin')) {
    platforms.value = ['douyin']
    message.info('点榜模式仅支持抖音，已自动切换到抖音平台')
  }
  searching.value = true
  selected.value.clear()
  try {
    ...
    results.value = (data.videos || []).map(v => ({
      ...
      _analysis: normalizeAnalysis(v.analysis, v),
    }))
    if (data.warnings?.length) {
      data.warnings.forEach(w => message.warning(w, { duration: 5000 }))
    }
    if (!results.value.length) {
      if (videoType.value && keyword.value.trim()) {
        message.info('榜中没有匹配该关键词的结果，可以清空关键词直接查看完整热')
      } else if (videoType.value) {
        message.info('当前热榜暂无数据，请稍后再试')
      } else {
        message.info('找到相关内容，请换个关键词试')
      }
    } else {
      message.success(`找到 ${results.value.length} 条爆款内容`)
    }
    _saveSearchCond()
...
async function saveToLibrary(v, options = {}) {
  ...
  try {
    ...
    v.id = data.id
    if (data.viral_reason) v.viral_reason = data.viral_reason
    v._saved = true
    if (!silent) {
...
async function submitUrlAdd() {
  ...
  try {
    const { data } = await topicsApi.addByUrl(url)
    if (data.already_saved) {
      message.info('该选题已在选题库中')
    } else {
      message.success('已保存到选题库')
    }
    showUrlModal.value = false
    urlInput.value = ''
    // 若当前抽屉开着，刷新一下
    if (showSavedDrawer.value) {
...
async function handleBatchDelete() {
  ...
  try {
    ...
    window.$message?.success(`已删${savedSelectedIds.value.length} 条`)
...
async function openSavedDrawer() {
  ...
  try {
    const { data } = await topicsApi.list({ limit: 200 })
    savedTopics.value = data.map(t => ({ ...t, _analysis: normalizeAnalysis(t.analysis, t) }))
...
async function analyzeSavedVideo(t) {
  ...
  try {
    ...
    if (data.script) t.script = data.script
    const mergedAnalysis = normalizeAnalysis(data, t)
    t._analysis = mergedAnalysis
    t.has_analysis = true
    currentAnalysis.value = mergedAnalysis
...
async function ensureAnalysisForView(item) {
  let merged = normalizeAnalysis(item._analysis, item)
  if (merged?.script || merged?.description) return merged

  if (item?.id) {
    try {
      const { data } = await topicsApi.getTopicAnalysis(item.id)
      merged = normalizeAnalysis(data, item)
      if (merged) {
        item._analysis = merged
        item._hasAnalysis = true
    ...
    if (!merged?.script && !merged?.description) {
      await fetchTopicDetail(item, { silent: true })
      merged = normalizeAnalysis(item._analysis, item) || merged
    }
  }
...
async function viewSavedAnalysis(t) {
  const merged = await ensureAnalysisForView(t)
  if (!merged) {
    message.warning('该选题暂无分析结果，请先点击「分析爆款」')
    return
  }
  currentAnalysis.value = merged
  currentAnalysisTitle.value = t.title || '(无标题)'
  showAnalysisModal.value = true
}
...
async function analyzeVideo(v) {
  ...
  if (!v.script || v.script === v.title) {
    message.info('正在获取视频语音内容，首次分析约 30-60 秒，请稍候...', { duration: 8000 })
  }
  try {
    const { data } = await topicsApi.analyzeTopic(v.id)
    if (data.script) v.script = data.script
    const mergedAnalysis = normalizeAnalysis(data, v)
    const idx = results.value.findIndex(r => r.id === v.id)
    if (idx !== -1) {
      results.value[idx] = {
        ...results.value[idx],
        _analysis: mergedAnalysis,
        _hasAnalysis: true,
        has_analysis: true,
      }
    }
...
async function viewAnalysis(v) {
  const merged = await ensureAnalysisForView(v)
  if (!merged) {
    message.warning('该选题暂无分析结果，请先点击「分析爆款」')
    return
  }
...
const pendingAddToDocsItems = ref([])
const addToDocsSource = ref('search-batch')

async function openAddToDocsModal(source = 'search-batch', items = []) {
  let selectedItems = Array.isArray(items) ? items.filter(Boolean) : []
  if (!selectedItems.length) {
    if (source === 'saved-batch') {
      selectedItems = savedTopics.value.filter(v => savedSelectedIds.value.includes(v.id))
    } else {
      selectedItems = results.value.filter(v => isSelected(v))
    }
  }
  if (!selectedItems.length) {
    message.warning('请先选择要加入资料库的内容')
    return
  }

  pendingAddToDocsItems.value = selectedItems
  addToDocsSource.value = source
  try {
...
async function confirmAddToDocs() {
  const selectedItems = pendingAddToDocsItems.value
  if (!selectedItems.length) return
  addingToDocs.value = true
  let ok = 0, skip = 0
  for (const item of selectedItems) {
    let content = (item.script || item.description || '').trim()
    if (!content) {
      await fetchTopicDetail(item, { silent: true })
      content = (item.script || item.description || '').trim()
    }
    if (!content) { skip++; continue }

    const sourceRef = item.video_id || (item.id ? String(item.id) : '')
    if (!sourceRef) { skip++; continue }

    try {
      await documentsApi.addText({
        name: `[爆款] ${(item.title || '未命名选题').slice(0, 40)}`,
        content,
  ...
  pendingAddToDocsItems.value = []
  addToDocsSource.value = 'search-batch'
  if (ok) message.success(`已添加 ${ok} 条文案到资料库`)
  if (skip) message.warning(`${skip} 条缺少可用文案，已跳过（可先获取视频文案后再试）`)
}
...
// 页面加载时：�?上次搜索条件，从�?库拉�??应结???
async function loadSearchResults() {
  if (!keyword.value) return  // 没搜索过，保持空�?
  try {
...
.page-subtitle { font-size: 13px; color: var(--c-text-3, #94a3b8); margin-top: 4px; }

/* Naive UI 输入�?选择框统一为青色系 */
:deep(.n-input .n-input__border),
:deep(.n-input .n-input__state-border) {
  border-color: #e0f2fe !important;
}
:deep(.n-input:not(.n-input--disabled):hover .n-input__border),
:deep(.n-input:not(.n-input--disabled):hover .n-input__state-border) {
...
/* n-input background */
:deep(.n-input:not(.n-input--focus) .n-input-wrapper) {
...
:deep(.n-base-selection:not(.n-base-selection--disabled):hover .n-base-selection__border),
:deep(.n-base-selection:not(.n-base-selection--disabled):hover .n-base-selection__state-border) {
  border-color: #7dd3fc !important;
}
:deep(.n-base-selection:not(.n-base-selection--disabled):not(.n-base-selection--active) .n-base-selection-input__content),
:deep(.n-base-selection .n-base-selection-label__input) {
  color: #0f172a !important;
}
:deep(.n-base-selection .n-base-selection__placeholder) {
  color: #94a3b8 !important;
}
/* n-form-item label (fallback for any remaining inline label) */
...
.section-title {
  font-size: 15px; font-weight: 600; color: var(--c-text-1, #1a1a1a);
  margin-bottom: 16px;
}
...
.video-card {
  ...
  overflow: hidden;
  cursor: pointer;
  transition: border-color .2s ease, transform .15s, box-shadow .2s;
  position: relative;
  box-shadow: 0 1px 3px rgba(8,145,178,0.05);
}
.video-card:hover { border-color: rgba(8,145,178,.4); transform: translateY(-2px); box-shadow: 0 4px 16px rgba(8,145,178,.12); }
.video-card.selected { border-color: var(--c-primary, #0891b2); background: rgba(8,145,178,.05); }
...
.video-title {
  font-size: 13px; font-weight: 500; color: var(--c-text-1, #1a1a1a);
  display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical;
  overflow: hidden; line-height: 1.4; margin-bottom: 6px;
}
.video-author { font-size: 12px; color: var(--c-text-4, #888); margin-bottom: 8px; }

.video-stats { display: flex; gap: 12px; margin-bottom: 8px; }
.stat { display: flex; align-items: center; gap: 3px; font-size: 12px; color: var(--c-text-3, #888); font-variant-numeric: tabular-nums; }
.ratio-tag { background: rgba(8,145,178,.1); color: var(--c-primary, #0891b2); border-radius: 4px; padding: 0 4px; font-size: 11px; }
...
.ratio-item { flex: 1; text-align: center; background: #f0f9ff; border-radius: 10px; padding: 14px 8px; border: 1px solid #e0f2fe; }
.ratio-label { font-size: 12px; color: #888; margin-bottom: 6px; }
.ratio-value { font-size: 22px; font-weight: 700; color: var(--c-text-1, #1a1a1a); margin-bottom: 8px; }
.analysis-label { font-size: 12px; color: #888; margin: 6px 0 10px; }
.analysis-script {
  max-height: 220px;
  overflow: auto;
  font-size: 13px;
...
.analysis-text { font-size: 14px; color: var(--c-text-2, #555); line-height: 1.8; padding: 8px 0; white-space: pre-wrap; }
.analysis-highlight { background: rgba(234,179,8,.1); border-left: 3px solid #eab308; padding: 12px 16px; border-radius: 6px; }
...
.st-card:hover { border-color: rgba(8,145,178,.45); box-shadow: 0 3px 12px rgba(8,145,178,.1); }
...
.st-cover { width: 100%; height: 130px; background: #f8f8fa; position: relative; overflow: hidden; display: flex; align-items: center; justify-content: center; }
...
.st-author { font-size: 12px; color: #888; margin-bottom: 6px; }
.st-stats { display: flex; gap: 10px; margin-bottom: 6px; flex-wrap: wrap; }
.st-stat { display: flex; align-items: center; gap: 3px; font-size: 12px; color: #888; }
.st-ratio { background: rgba(8,145,178,.1); color: #0891b2; border-radius: 4px; padding: 0 4px; font-size: 11px; }
.st-viral-reason {
  margin: 6px 0;
  font-size: 12px;
  line-height: 1.55;
...
.pull-progress { display: flex; align-items: center; padding: 12px 0; border-top: 1px solid rgba(0,0,0,.06); margin-top: 12px; }

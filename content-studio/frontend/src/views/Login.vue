  ...
  <div class="login-shell">
    ...
    <div class="login-panel">
      <div class="login-card">
        ...
        <!-- OAuth 按钮 -->
        <div class="oauth-buttons" v-if="googleEnabled || githubEnabled">
          <button v-if="googleEnabled" class="oauth-btn google" @click="handleGoogleLogin" :disabled="oauthLoading">
          ...
          <button v-if="githubEnabled" class="oauth-btn github" @click="handleGithubLogin" :disabled="oauthLoading">
        ...
        <!-- 分隔符 -->
        <div class="divider" v-if="googleEnabled || githubEnabled">
          <span>或使用邮箱</span>
        </div>
...
onMounted(async () => {
  try {
    ...
    // 旧后端不返回这两个字段时保持按钮可见，避免误藏入口。
    googleEnabled.value = data.google_enabled === undefined ? true : !!data.google_enabled
    githubEnabled.value = data.github_enabled === undefined ? true : !!data.github_enabled

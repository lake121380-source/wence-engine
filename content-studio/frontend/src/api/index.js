...
// 延迟获取 subscription store，避免 Pinia 未初始化时调用
let _getSubscriptionStore = null
export function setSubscriptionStoreGetter(getter) { _getSubscriptionStore = getter }
...
api.interceptors.response.use(
  ...
  err => {
    ...
    if (err.response?.status === 402) {
      // 优先弹窗提示，不直接跳转
      if (_getSubscriptionStore) {
        try {
          const store = _getSubscriptionStore()
          store.triggerExpired()
        } catch {}
      }
    }
...
export const authApi = {
  ...
  resendVerifyEmail: (data) => api.post('/auth/email/resend', data),
  // Google / GitHub OAuth
...
export const generateApi = {
  ...
  list: (params = {}) => {
    ...
    qs.append('limit', String(query.limit ?? 20))
    if (query.offset != null) qs.append('offset', String(query.offset))
    if (query.platform) qs.append('platform', query.platform)
    if (query.q) qs.append('q', query.q)
    if (query.rating != null) qs.append('rating', String(query.rating))
    if (query.liked_only) qs.append('liked_only', 'true')
    return api.get(`/generations?${qs.toString()}`)
  },
...
export const topicsApi = {
  ...
  addByUrl: (url) => api.post('/topics/add-by-url', { url }),
